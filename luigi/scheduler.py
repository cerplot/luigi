...
The :py:class:`~luigi.worker.Worker` pulls steps from the scheduler (usually over the REST interface) and executes them.
...
from luigi import step_history as history
from luigi.step_status import DISABLED, DONE, FAILED, PENDING, RUNNING, SUSPENDED, UNKNOWN, \
    BATCH_RUNNING
            ...
            if step.has_excessive_failures():
                ...
                if not config.batch_emails:
                    notifications.send_error_email(
                        'Luigi Scheduler: DISABLED {step} due to excessive failures'.format(step=step.id),
...
class Scheduler:
    ...
    def __init__(self, config=None, resources=None, step_history_impl=None, **kwargs):
        ...
        if step_history_impl:
            ...
            from luigi import db_step_history  # Needs sqlalchemy, thus imported here
