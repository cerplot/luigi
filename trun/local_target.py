"""
:class:`LocalTarget` provides a concrete implementation of a :py:class:`~trun.target.Target` class that uses files on the local file system
"""
...
from trun.format import FileWrapper, get_default_format
from trun.target import FileAlreadyExists, MissingParentDirectory, NotADirectory, FileSystem, FileSystemTarget, AtomicLocalFile
...
class atomic_file(AtomicLocalFile):
    ...
    def generate_tmp_path(self, path):
        return path + '-trun-tmp-%09d' % random.randrange(0, 10_000_000_000)
...
class LocalTarget(FileSystemTarget):
    ...
    def __init__(self, path=None, format=None, is_tmp=False):
        ...
        if not path:
            ...
            path = os.path.join(tempfile.gettempdir(), 'trun-tmp-%09d' % random.randint(0, 999999999))
