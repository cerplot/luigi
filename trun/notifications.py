...
In particular using the config `receiver` should set up Trun so that it will send emails when steps fail.
...
DEFAULT_CLIENT_EMAIL = 'trun-client@%s' % socket.gethostname()


class TestNotificationsStep(trun.step.Step):
    """
    ...
    .. code-block:: console
            ...
            $ trun TestNotificationsStep --local-scheduler --email-force-send

    And then check your email inbox to see if you got an error email or any
    other kind of notifications that you expected.
    """
    raise_in_complete = trun.parameter.BoolParameter(description='If true, fail in complete() instead of run()')
...
def format_step_error(headline, step, command, formatted_exception=None):
    """
    Format a message body for an error email related to a trun.step.Step

    :param headline: Summary line for the message
    :param step: `trun.step.Step` instance where this error occurred
