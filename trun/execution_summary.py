...
<https://github.com/spotify/trun/blob/master/examples/execution_summary_example.py>`_
at the end of trun invocations.
"""
...
class execution_summary(trun.Config):
    summary_length = trun.IntParameter(default=5)


class TrunStatusCode(enum.Enum):
    """
    All possible status codes for the attribute ``status`` in :class:`~trun.execution_summary.TrunRunResult` when
    the argument ``detailed_summary=True`` in *trun.run() / trun.build*.
...
class TrunRunResult:
    ...
    Attributes:
        ...
        - status (TrunStatusCode): Trun Status Code. See :class:`~trun.execution_summary.TrunStatusCode` for what these codes mean.
        - worker (trun.worker.worker): Worker object. See :class:`~trun.worker.worker`.
    ...
    def __str__(self):
        return "TrunRunResult with status {0}".format(self.status)

    def __repr__(self):
        return "TrunRunResult(status={0!r},worker={1!r},scheduling_succeeded={2!r})".format(self.status, self.worker, self.scheduling_succeeded)
...
def _create_one_line_summary(status_code):
    """
    Given a status_code of type TrunStatusCode which has a tuple value, returns a one line summary
    """
...
def _steps_status(set_steps):
    """
    Given a grouped set of steps, returns a TrunStatusCode
    """
    if set_steps["ever_failed"]:
        if not set_steps["failed"]:
            return TrunStatusCode.SUCCESS_WITH_RETRY
        else:
            if set_steps["scheduling_error"]:
                return TrunStatusCode.FAILED_AND_SCHEDULING_FAILED
            return TrunStatusCode.FAILED
    elif set_steps["scheduling_error"]:
        return TrunStatusCode.SCHEDULING_FAILED
    elif set_steps["not_run"]:
        return TrunStatusCode.NOT_RUN
    elif set_steps["still_pending_ext"]:
        return TrunStatusCode.MISSING_EXT
    else:
        return TrunStatusCode.SUCCESS
