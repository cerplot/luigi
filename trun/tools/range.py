...
from trun.target import FileSystemTarget
from trun.step import Register, flatten_output


logger = logging.getLogger('trun-interface')


class RangeEvent(trun.Event):  # Not sure if subclassing currently serves a purpose. Stringly typed, events are.
    """
...
class RangeBase(trun.WrapperStep):
    """
    ...
    of = trun.StepParameter(
        description="step name to be completed. The step must take a single datetime parameter")
    of_params = trun.DictParameter(default=dict(), description="Arguments to be provided to the 'of' class when instantiating")
    ...
    start = trun.Parameter()
    stop = trun.Parameter()
    reverse = trun.BoolParameter(
        default=False,
...
class RangeDailyBase(RangeBase):
    ...
    days_forward = trun.IntParameter(
        default=0,
...
class RangeHourlyBase(RangeBase):
    ...
    hours_forward = trun.IntParameter(
        default=0,
    ...
    def _format_datetime(self, dt):
        return trun.DateHourParameter().serialize(dt)
...
class RangeByMinutesBase(RangeBase):
    ...
    minutes_back = trun.IntParameter(
        ...
        description=("extent to which contiguousness is to be assured into "
                     ...
                     "outputs flapping. Increase freely if you intend to "
                     "process old dates - worker's memory is the limit"))
    minutes_forward = trun.IntParameter(
        default=0,
    ...
    minutes_interval = trun.IntParameter(
        default=1,
    ...
    def _format_datetime(self, dt):
        return trun.DateMinuteParameter().serialize(dt)
...
def _get_filesystems_and_globs(datetime_to_step, datetime_to_re):
    ...
    For convenience, the step can be a trun.WrapperStep,
...
def _list_existing(filesystem, glob, paths):
    ...
    Takes a trun.target.FileSystem object, a str which represents a glob and
...
def infer_bulk_complete_from_fs(datetimes, datetime_to_step, datetime_to_re):
    ...
    (Eventually Trun could have ranges of completion as first-class citizens.
...
class RangeMonthly(RangeBase):
    ...
    months_back = trun.IntParameter(
        ...
        description=("extent to which contiguousness is to be assured into "
                     ...
                     "flapping. Increase freely if you intend to process old "
                     "dates - worker's memory is the limit"))
    months_forward = trun.IntParameter(
        default=0,
...
class RangeDaily(RangeDailyBase):
    ...
    .. code-block:: console
        ...
        trun --module your.module RangeDaily --of YourActualStep --start 2014-01-01
    """
...
class RangeHourly(RangeHourlyBase):
    ...
    .. code-block:: console
        ...
        trun --module your.module RangeHourly --of YourActualStep --start 2014-01-01T00
    """
...
class RangeByMinutes(RangeByMinutesBase):
    ...
    .. code-block:: console
        ...
        trun --module your.module RangeByMinutes --of YourActualStep --start 2014-01-01T0123
    """
