...
"""
trun.configuration provides some convenience wrappers around Python's
ConfigParser to get configuration options from config files.

The default location for configuration files is trun.cfg (or client.cfg) in the current
working directory, then /etc/trun/client.cfg.
...
class TrunConfigParser(BaseParser, ConfigParser):
    NO_DEFAULT = object()
    enabled = True
    optionxform = str
    _instance = None
    _config_paths = [
        '/etc/trun/client.cfg',  # Deprecated old-style global trun config
        '/etc/trun/trun.cfg',
        'client.cfg',  # Deprecated old-style local trun config
        'trun.cfg',
    ]
    ...
    def reload(cls):
        ...
        if deprecated_paths:
            warnings.warn("Trun configuration files named 'client.cfg' are deprecated if favor of 'trun.cfg'. " +
    ...
    def _get_with_default(self, method, section, option, default, expected_type=None, **kwargs):
        ...
        try:
            ...
            if default is TrunConfigParser.NO_DEFAULT:
                raise
