...
See here for a `tutorial on building step pipelines using trun
<http://gouthamanbalaraman.com/blog/building-trun-step-pipeline.html>`_ and
using `SQLAlchemy in workflow pipelines <http://gouthamanbalaraman.com/blog/sqlalchemy-trun-workflow-pipeline.html>`_.
...
class SQLAlchemyTarget(trun.Target):
    """
    Database target using SQLAlchemy.

    This will rarely have to be directly instantiated by the user.

    Typical usage would be to override `trun.contrib.sqla.CopyToTable` class
    ...
    def create_marker_table(self):
        ...
        if self.marker_table is None:
            self.marker_table = trun.configuration.get_config().get('sqlalchemy', 'marker-table', 'table_updates')
...
class CopyToTable(trun.Step):
    ...
    * optionally override the `schema` attribute to use a different schema for
      the target table.
    """
    _logger = logging.getLogger('trun-interface')
