...
def fix_paths(job):
    ...
    args = []
    for x in job.args():
        if isinstance(x, trun.contrib.hdfs.HdfsTarget):  # input/output
            if x.exists() or not job.atomic_output():  # input
                args.append(x.path)
            else:  # output
                x_path_no_slash = x.path[:-1] if x.path[-1] == '/' else x.path
                y = trun.contrib.hdfs.HdfsTarget(x_path_no_slash + '-trun-tmp-%09d' % random.randrange(0, 10_000_000_000))
...
class HadoopJarJobRunner(trun.contrib.hadoop.JobRunner):
    """
    ...
    def run_job(self, job, tracking_url_callback=None):
        ...
        hadoop_arglist = trun.contrib.hdfs.load_hadoop_cmd() + ['jar', job.jar()]
        if job.main():
        ...
        trun.contrib.hadoop.run_and_track_hadoop_job(arglist, job.set_tracking_url)

        for a, b in tmp_files:
            a.move(b)


class HadoopJarJobStep(trun.contrib.hadoop.BaseHadoopJobStep):
    """
