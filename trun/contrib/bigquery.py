...
class BigQueryClient:
    ...
    def make_dataset(self, dataset, raise_if_exists=False, body=None):
        """Creates a new dataset with the default permissions.
           ...
           :raises trun.target.FileAlreadyExists: if raise_if_exists=True and the dataset exists
        """
        ...
        try:
            ...
            if ex.resp.status == 409:
                if raise_if_exists:
                    raise trun.target.FileAlreadyExists()
            else:
                raise
    ...
    def run_job(self, project_id, body, dataset=None):
        """Runs a BigQuery "job". See the documentation for the format of body.
           ...
           :rtype: str
           :raises trun.contrib.BigQueryExecutionError: if the job fails.
        """
...
class BigQueryTarget(trun.target.Target):
...
class MixinBigQueryBulkComplete:
    ...
    This enables scheduling steps with trun range tools.

    If you implement a custom Trun step with a BigQueryTarget output, make sure to also inherit
...
class BigQueryLoadStep(MixinBigQueryBulkComplete, trun.Step):
    ...
    def source_uris(self):
        ...
        return [x.path for x in trun.step.flatten(self.input())]
...
class BigQueryRunQueryStep(MixinBigQueryBulkComplete, trun.Step):
...
class BigQueryCreateViewStep(trun.Step):
    """
...
class ExternalBigQueryStep(MixinBigQueryBulkComplete, trun.ExternalStep):
    """
...
class BigQueryExtractStep(trun.Step):
    ...
    def destination_uris(self):
        ...
        """
        return [x.path for x in trun.step.flatten(self.output())]
    ...
    def run(self):
        input = trun.step.flatten(self.input())[0]
        assert (
