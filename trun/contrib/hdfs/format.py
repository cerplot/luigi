...
import trun.format
from trun.contrib.hdfs.config import load_hadoop_cmd
from trun.contrib.hdfs import config as hdfs_config
from trun.contrib.hdfs.clients import remove, rename, mkdir, listdir, exists
from trun.contrib.hdfs.error import HDFSCliError
...
class HdfsReadPipe(trun.format.InputPipeProcessWrapper):

    def __init__(self, path):
        super(HdfsReadPipe, self).__init__(load_hadoop_cmd() + ['fs', '-cat', path])


class HdfsAtomicWritePipe(trun.format.OutputPipeProcessWrapper):
    """
...
class HdfsAtomicWriteDirPipe(trun.format.OutputPipeProcessWrapper):
    """
...
class PlainFormat(trun.format.Format):
...
class PlainDirFormat(trun.format.Format):
...
class CompatibleHdfsFormat(trun.format.Format):
