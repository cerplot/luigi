...
from trun.contrib.hdfs.config import tmppath
...
class HdfsTarget(FileSystemTarget):
    ...
    def __init__(self, path=None, format=None, is_tmp=False, fs=None):
        ...
        if format is None:
            format = trun.format.get_default_format() >> hdfs_format.Plain
    ...
    def move_dir(self, path):
        """
        Move using :py:class:`~trun.contrib.hdfs.abstract_client.HdfsFileSystem.rename_dont_move`

        New since after trun v2.1: Does not change self.path
        ...
        with that over just using plain mv.  See spotify/trun#557
        """
