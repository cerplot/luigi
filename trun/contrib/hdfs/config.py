...
class hdfs(trun.Config):
    client_version = trun.IntParameter(default=None)
    namenode_host = trun.OptionalParameter(default=None)
    namenode_port = trun.IntParameter(default=None)
    client = trun.Parameter(default='hadoopcli')
    tmp_dir = trun.OptionalParameter(
        default=None,
        config_path=dict(section='core', name='hdfs-tmp-dir'),
    )


class hadoopcli(trun.Config):
    command = trun.Parameter(default="hadoop",
    ...
    version = trun.Parameter(default="cdh4",
...
def tmppath(path=None, include_unix_username=True):
    ...
    """
    addon = "truntemp-%09d" % random.randrange(0, 10_000_000_000)
