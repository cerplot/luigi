...
"""
Provides access to HDFS using the :py:class:`HdfsTarget`, a subclass of :py:class:`~trun.target.Target`.
...
Since the hdfs functionality is quite big in trun, it's split into smaller
files under ``trun/contrib/hdfs/*.py``. But for the sake of convenience and
API stability, everything is reexported under :py:mod:`trun.contrib.hdfs`.
"""

# imports
from trun.contrib.hdfs import config as hdfs_config
from trun.contrib.hdfs import clients as hdfs_clients
from trun.contrib.hdfs import error as hdfs_error
from trun.contrib.hdfs import hadoopcli_clients as hdfs_hadoopcli_clients
from trun.contrib.hdfs import webhdfs_client as hdfs_webhdfs_client
from trun.contrib.hdfs import format as hdfs_format
from trun.contrib.hdfs import target as hdfs_target
