...
"""
A trun file system client that wraps around the hdfs-library (a webhdfs
client)
...
from trun.contrib.hdfs import config as hdfs_config
from trun.contrib.hdfs import abstract_client as hdfs_abstract_client
import trun.contrib.target
import logging
import os
import warnings
...
class webhdfs(trun.Config):
    port = trun.IntParameter(default=50070,
                              description='Port for webhdfs')
    user = trun.Parameter(default='', description='Defaults to $USER envvar',
                           config_path=dict(section='hdfs', name='user'))
    client_type = trun.ChoiceParameter(var_type=str, choices=['insecure', 'kerberos'],
                                        default='insecure', description='Type of hdfs client to use.')


class WebHdfsClient(hdfs_abstract_client.HdfsFileSystem):
    """
    A webhdfs that tries to confirm to truns interface for file existence.
    ...
    def chmod(self, path, permissions, recursive=False):
        ...
        """
        raise NotImplementedError("Webhdfs in trun doesn't implement chmod")

    def chown(self, path, owner, group, recursive=False):
        """
        Raise a NotImplementedError exception.
        """
        raise NotImplementedError("Webhdfs in trun doesn't implement chown")

    def count(self, path):
        """
        Raise a NotImplementedError exception.
        """
        raise NotImplementedError("Webhdfs in trun doesn't implement count")

    def copy(self, path, destination):
        """
        Raise a NotImplementedError exception.
        """
        raise NotImplementedError("Webhdfs in trun doesn't implement copy")
