...
from trun.target import FileAlreadyExists, FileSystemTarget
...
def load_hive_cmd():
    return trun.configuration.get_config().get('hive', 'command', 'hive').split(' ')


def get_hive_syntax():
    return trun.configuration.get_config().get('hive', 'release', 'cdh4')


def get_hive_warehouse_location():
    return trun.configuration.get_config().get('hive', 'warehouse_location', '/user/hive/warehouse')


def get_ignored_file_masks():
    return trun.configuration.get_config().get('hive', 'ignored_file_masks', None)
...
class HiveQueryStep(trun.contrib.hadoop.BaseHadoopJobStep):
    """
    ...
    def hiverc(self):
        ...
        if hiverc-location key is specified in trun.cfg, will default to the value there
        otherwise returns None.

        Returning a list of rc files will load all of them in order.
        """
        return trun.configuration.get_config().get('hive', 'hiverc-location', default=None)
    ...
    def hiveconfs(self):
        ...
        * hive.exec.reducers.max (reducers_max)
        ...
        if self.pool is not None:
            ...
            scheduler_type = trun.configuration.get_config().get('hadoop', 'scheduler', 'fair')
...
class HiveQueryRunner(trun.contrib.hadoop.JobRunner):
    """
    ...
    def run_job(self, job, tracking_url_callback=None):
        ...
        with tempfile.NamedTemporaryFile() as f:
            ...
            return trun.contrib.hadoop.run_and_track_hadoop_job(arglist, job.set_tracking_url)


class HivePartitionTarget(trun.Target):
    """
...
class ExternalHiveStep(trun.ExternalStep):
    """
    External step that depends on a Hive table/partition.
    """
    database = trun.Parameter(default='default')
    table = trun.Parameter()
    partition = trun.DictParameter(
        default={},
