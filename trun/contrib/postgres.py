"""
Implements a subclass of :py:class:`~trun.target.Target` that writes data to Postgres.
...
DB_DRIVER = os.environ.get('TRUN_PGSQL_DRIVER', 'psycopg2')
...
class PostgresTarget(trun.Target):
    """
    Target for a resource in Postgres.

    This will rarely have to be directly instantiated by the user.
    """
    marker_table = trun.configuration.get_config().get('postgres', 'marker-table', 'table_updates')
...
class CopyToTable(rdbms.CopyToTable):
    ...
    def run(self):
        ...
        tmp_dir = trun.configuration.get_config().get('postgres', 'local-tmp-dir', None)
