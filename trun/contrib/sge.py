...
`LSF extension <https://github.com/dattalab/trun/blob/lsf/trun/lsf.py>`_
...
using ``qstat``. To get started, install trun on all nodes.

To run trun workflows on an SGE cluster, subclass
:class:`trun.contrib.sge.SGEJobStep` as you would any :class:`trun.Step`,
but override the ``work()`` method, instead of ``run()``, to define the job
code. Then, run your Trun workflow from the master node, assigning > 1
...
.. code-block:: python
    ...
    import trun
    import os
    from trun.contrib.sge import SGEJobStep
    ...
    class TestJobStep(SGEJobStep):
        ...
        def output(self):
            return trun.LocalTarget(os.path.join('/home', 'testfile_' + str(self.i)))
...
:class:`trun.contrib.sge.SGEJobStep` for other SGE parameters. As for any
step, you can also set these in your trun configuration file as shown below.
...
import trun
from trun.contrib.hadoop import create_packages_archive
from trun.contrib import sge_runner
...
class SGEJobStep(trun.Step):
    ...
    - no_tarball: Don't create a tarball of the trun project directory.  Can be
        useful to reduce I/O requirements when the trun directory is accessible
        from cluster nodes already.

    """

    n_cpu = trun.IntParameter(default=2, significant=False)
    shared_tmp_dir = trun.Parameter(default='/home', significant=False)
    parallel_env = trun.Parameter(default='orte', significant=False)
    job_name_format = trun.Parameter(
        significant=False, default=None, description="A string that can be "
        "formatted with class variables to name the job with qsub.")
    job_name = trun.Parameter(
        significant=False, default=None,
        description="Explicit job name given via qsub.")
    run_locally = trun.BoolParameter(
        significant=False,
        description="run locally instead of on the cluster")
    poll_time = trun.IntParameter(
        significant=False, default=POLL_TIME,
        description="specify the wait time to poll qstat for the job status")
    dont_remove_tmp_dir = trun.BoolParameter(
        significant=False,
        description="don't delete the temporary directory used (for debugging)")
    no_tarball = trun.BoolParameter(
        significant=False,
        description="don't tarball (and extract) the trun project files")
    ...
    def _init_local(self):
        ...
        if not self.no_tarball:
            ...
            # This is not necessary if trun is importable from the cluster node
            logging.debug("Tarballing dependencies")
            # Grab trun and the module containing the code to be run
            packages = [trun] + [__import__(self.__module__, None, None, 'dummy')]
