...
to subclass :py:class:`trun.contrib.hadoop.JobStep` and implement a
...
import trun
import trun.step
import trun.contrib.gcs
import trun.contrib.hdfs
import trun.contrib.s3
...
class hadoop(trun.step.Config):
    pool = trun.OptionalParameter(
        default=None,
        description=(
...
def run_and_track_hadoop_job(arglist, tracking_url_callback=None, env=None):
    ...
    def write_trun_history(arglist, history):
        """
        ...
        if history_filename and '-output' in arglist:
            ...
            f = trun.contrib.hdfs.HdfsTarget(os.path.join(output_dir, history_filename)).open('w')
    ...
    def track_process(arglist, tracking_url_callback, env=None):
        ...
        if proc.returncode == 0:
            write_trun_history(arglist, {'job_id': job_id})
            return (out, err)
...
def fetch_step_failures(tracking_url):
    ...
    for link in links:
        ...
        for exc in re.findall(r'trun-exc-hex=[0-9a-f]+', data):
...
class HadoopJobRunner(JobRunner):
    ...
    def run_job(self, job, tracking_url_callback=None):
        ...
        packages = [trun] + self.modules + job.extra_modules() + list(_attached_packages)
        ...
        if base_tmp_dir:
            warnings.warn("The core.tmp-dir configuration item is"
                          ...
                          " to control where trun.contrib.hadoop may"
        ...
        if self.end_job_with_atomic_move_dir:
            illegal_targets = (
                trun.contrib.s3.S3FlagTarget, trun.contrib.gcs.GCSFlagTarget)
        ...
        arglist = trun.contrib.hdfs.load_hadoop_cmd() + ['jar', self.streaming_jar]

        # 'libjars' is a generic option, so place it first
        libjars = [libjar for libjar in self.libjars]

        for libjar in self.libjars_in_hdfs:
            run_cmd = trun.contrib.hdfs.load_hadoop_cmd() + ['fs', '-get', libjar, self.tmp_dir]
        ...
        allowed_input_targets = (
            trun.contrib.hdfs.HdfsTarget,
            trun.contrib.s3.S3Target,
            trun.contrib.gcs.GCSTarget)
        for target in trun.step.flatten(job.input_hadoop()):
        ...
        allowed_output_targets = (
            trun.contrib.hdfs.HdfsTarget,
            trun.contrib.s3.S3FlagTarget,
            trun.contrib.gcs.GCSFlagTarget)
        ...
        if self.end_job_with_atomic_move_dir:
            trun.contrib.hdfs.HdfsTarget(output_hadoop).move_dir(output_final)
        self.finish()
...
class LocalJobRunner(JobRunner):
    ...
    def run_job(self, job):
        ...
        for i in trun.step.flatten(job.input_hadoop()):
...
class BaseHadoopJobStep(trun.Step):
    pool = trun.OptionalParameter(default=None, significant=False, positional=False)
    ...
    def input_local(self):
        return trun.step.getpaths(self.requires_local())

    def input_hadoop(self):
        return trun.step.getpaths(self.requires_hadoop())

    def deps(self):
        # Overrides the default implementation
        return trun.step.flatten(self.requires_hadoop()) + trun.step.flatten(self.requires_local())
...
class JobStep(BaseHadoopJobStep):
    ...
    def job_runner(self):
        ...
        """
        outputs = trun.step.flatten(self.output())
        for output in outputs:
            if not isinstance(output, trun.contrib.hdfs.HdfsTarget):
