...
"""
Kubernetes Job wrapper for Trun.
...
class KubernetesJobStep(trun.Step):
    ...
    def spec_schema(self):
        ...
        **restartPolicy**
        ...
        - **Warning**: restartPolicy=OnFailure will bypass max_retrials, and restart
          the container until success, with the risk of blocking the Trun step.
    ...
    def __get_job(self):
        jobs = Job.objects(self.__kube_api, namespace=self.kubernetes_namespace) \
            .filter(selector="trun_step_id=" + self.job_uuid) \
            .response['items']
    ...
    def output(self):
        ...
        Example::
            ...
            return trun.LocalTarget(os.path.join('/tmp', 'example'))
