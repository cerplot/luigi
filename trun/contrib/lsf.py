...
from trun.contrib import lsf_runner
from trun.step_status import PENDING, FAILED, DONE, RUNNING, UNKNOWN
...
See: https://github.com/spotify/trun/issues/1936
...
class LSFJobStep(trun.Step):
    """
    Takes care of uploading and executing an LSF job
    """

    n_cpu_flag = trun.IntParameter(default=2, significant=False)
    shared_tmp_dir = trun.Parameter(default='/tmp', significant=False)
    resource_flag = trun.Parameter(default='mem=8192', significant=False)
    memory_flag = trun.Parameter(default='8192', significant=False)
    queue_flag = trun.Parameter(default='queue_name', significant=False)
    runtime_flag = trun.IntParameter(default=60)
    job_name_flag = trun.Parameter(default='')
    poll_time = trun.FloatParameter(
        significant=False, default=5,
        description="specify the wait time to poll bjobs for the job status")
    save_job_info = trun.BoolParameter(default=False)
    output = trun.Parameter(default='')
    extra_bsub_args = trun.Parameter(default='')
