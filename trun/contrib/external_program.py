...
"""
Template steps for running external programs as trun steps.
...
`plumbum`_, and wrapping plumbum commands in normal trun
:py:class:`~trun.step.Step` s.
...
class ExternalProgramStep(trun.Step):
    """
    Template step for running an external program in a subprocess
    ...
    control the environment variables (see :py:class:`ExternalPythonProgramStep`
    for an example).
    ...
    capture_output = trun.BoolParameter(default=True, significant=False, positional=False)

    stream_for_searching_tracking_url = trun.parameter.ChoiceParameter(
    ...
    tracking_url_pattern = trun.OptionalParameter(
    ...
    def program_args(self):
        """
        Override this method to map your step parameters to the program arguments
    ...
    def build_tracking_url(self, logs_output):
        ...
        :return: a tracking URL for the step
        """
        return logs_output
...
class ExternalPythonProgramStep(ExternalProgramStep):
    """
    Template step for running an external Python program in a subprocess

    Simple extension of :py:class:`ExternalProgramStep`, adding two
    :py:class:`trun.parameter.Parameter` s for setting a virtualenv and for
    extending the ``PYTHONPATH``.
    """
    virtualenv = trun.OptionalParameter(
        default=None,
        positional=False,
    ...
    extra_pythonpath = trun.OptionalParameter(
        default=None,
        positional=False,
    ...
    def program_environment(self):
        env = super(ExternalPythonProgramStep, self).program_environment()
