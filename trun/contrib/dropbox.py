...
import trun.format
from trun.target import FileSystem, FileSystemTarget, AtomicLocalFile
...
class DropboxClient(FileSystem):
    ...
    def __init__(self, token, user_agent="Trun"):
        """
...
class DropboxTarget(FileSystemTarget):
    ...
    def __init__(self, path, token, format=None, user_agent="Trun"):
        """
        ...
        **About the token parameter:**
        ...
        :param trun.Format format: the trun format to use (e.g. `trun.format.Nop`)
        ...
        self.format = format or trun.format.get_default_format()
