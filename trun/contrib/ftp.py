...
import trun
import trun.local_target
import trun.format
import trun.target
from trun.format import FileWrapper
...
class RemoteFileSystem(trun.target.FileSystem):
    ...
    def _sftp_put(self, local_path, path, atomic):
        ...
        if atomic:
            tmp_path = os.path.join(directory, 'trun-tmp-{:09d}'.format(random.randrange(0, 10_000_000_000)))
        else:
            tmp_path = normpath
    ...
    def _ftp_put(self, local_path, path, atomic):
        ...
        # random file name
        if atomic:
            tmp_path = folder + os.sep + 'trun-tmp-%09d' % random.randrange(0, 10_000_000_000)
        else:
            tmp_path = normpath
...
class AtomicFtpFile(trun.target.AtomicLocalFile):
    """
        ...
        if mode == 'w':
            ...
            try:
                ...
                self.__tmp_path = os.path.join(
                    tempfile.gettempdir(), 'trun-contrib-ftp', temppath
                )
            else:
