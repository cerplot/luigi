...
Modeled after :class:`trun.contrib.rdbms.CopyToTable`.
...
There are a two trun `trun.cfg` configuration options:
...
class ElasticsearchTarget(trun.Target):
    """ Target for a resource in Elasticsearch."""

    marker_index = trun.configuration.get_config().get('elasticsearch',
                                                        'marker-index', 'update_log')
    marker_doc_type = trun.configuration.get_config().get('elasticsearch',
                                                           'marker-doc-type', 'entry')
