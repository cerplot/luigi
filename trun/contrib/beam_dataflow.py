...
import trun
from trun.contrib import bigquery, gcs
...
class BeamDataflowJobStep(MixinNaiveBulkComplete, trun.Step, metaclass=abc.ABCMeta):
    """
    Trun wrapper for a Dataflow job. Must be overridden for each Beam SDK
    ...
    def dataflow_executable(self):
        ...
        return ['java', 'com.spotify.trun.MyClass', '-Xmx256m']
        """
        pass
    ...
    def _format_input_args(self):
        ...
        if isinstance(job_input, trun.Target):
        ...
        for (name, targets) in job_input.items():
            uris = [
              self.get_target_path(uri_target) for uri_target in trun.step.flatten(targets)
            ]
    ...
    def _format_output_args(self):
        ...
        if isinstance(job_output, trun.Target):
    ...
    def get_target_path(target):
        """
            Given a trun Target, determine a stringly typed path to pass as a
            Dataflow job argument.
        """
        if isinstance(target, trun.LocalTarget) or isinstance(target, gcs.GCSTarget):
            return target.path
