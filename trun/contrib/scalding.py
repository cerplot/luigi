...
"""
Scalding support for Trun.

Example configuration section in trun.cfg::
...
class ScaldingJobRunner(trun.contrib.hadoop.JobRunner):
    """
    JobRunner for `pyscald` commands. Used to run a ScaldingJobStep.
    """

    def __init__(self):
        conf = trun.configuration.get_config()
    ...
    def get_scalding_core(self):
        ...
        for j in os.listdir(lib_dir):
            if j.startswith('scalding-core-'):
                ...
                return p
        raise trun.contrib.hadoop.HadoopJobError('Could not find scalding-core.')
    ...
    def get_job_class(self, source):
        ...
        if job_class:
            ...
            raise trun.contrib.hadoop.HadoopJobError('Coudl not find scalding job class.')
    ...
    def run_job(self, job, tracking_url_callback=None):
        ...
        arglist = trun.contrib.hdfs.load_hadoop_cmd() + ['jar', scalding_core, '-libjars', libjars]
        ...
        (tmp_files, job_args) = trun.contrib.hadoop_jar.fix_paths(job)
        arglist += job_args
        ...
        trun.contrib.hadoop.run_and_track_hadoop_job(arglist, job.set_tracking_url, env=env)

        for a, b in tmp_files:
            a.move(b)


class ScaldingJobStep(trun.contrib.hadoop.BaseHadoopJobStep):
    """
