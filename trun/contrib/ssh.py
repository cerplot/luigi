...
trun-specific in the examples directory (examples/ssh_remote_execution.py)
...
the trun :class:`~trun.target.FileSystemTarget` semantics.
...
class RemoteFileSystem(trun.target.FileSystem):
    ...
    def mkdir(self, path, parents=True, raise_if_exists=False):
        if self.exists(path):
            if raise_if_exists:
                ...
                raise trun.target.NotADirectory()
            else:
                return
        ...
        try:
            ...
            if b'no such file' in e.output.lower():
                raise trun.target.MissingParentDirectory()
            raise
    ...
    def put(self, local_path, path):
        ...
        tmp_path = path + '-trun-tmp-%09d' % random.randrange(0, 10_000_000_000)
    ...
    def get(self, path, local_path):
        ...
        tmp_local_path = local_path + '-trun-tmp-%09d' % random.randrange(0, 10_000_000_000)
        self._scp("%s:%s" % (self.remote_context._host_ref(), path), tmp_local_path)
        os.rename(tmp_local_path, local_path)


class AtomicRemoteFileWriter(trun.format.OutputPipeProcessWrapper):
    ...
    def __init__(self, fs, path):
        ...
        self.__tmp_path = self.path + '-trun-tmp-%09d' % random.randrange(0, 10_000_000_000)
...
class RemoteTarget(trun.target.FileSystemTarget):
    """
    ...
    def __init__(self, path, host, format=None, **kwargs):
        ...
        if format is None:
            format = trun.format.get_default_format()
        self.format = format
    ...
    def open(self, mode='r'):
        if mode == 'w':
            ...
            file_reader = trun.format.InputPipeProcessWrapper(
