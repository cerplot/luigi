...
class salesforce(trun.Config):
    ...
    """
    username = trun.Parameter(default='')
    password = trun.Parameter(default='')
    security_token = trun.Parameter(default='')

    # sandbox token
    sb_security_token = trun.Parameter(default='')
...
class SalesforceAPI:
    ...
    def query_all(self, query, **kwargs):
        ...
        tmp_dir = trun.configuration.get_config().get('salesforce', 'local-tmp-dir', None)
