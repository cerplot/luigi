"""trun bindings for Google Dataproc on Google Cloud"""

import os
import time
import logging
import trun
from trun.contrib import gcp
...
class _DataprocBaseStep(trun.Step):
    gcloud_project_id = trun.Parameter(significant=False, positional=False)
    dataproc_cluster_name = trun.Parameter(significant=False, positional=False)
    dataproc_region = trun.Parameter(default="global", significant=False, positional=False)
...
class DataprocSparkStep(DataprocBaseStep):
    ...
    """
    main_class = trun.Parameter()
    jars = trun.Parameter(default="")
    job_args = trun.Parameter(default="")
...
class DataprocPysparkStep(DataprocBaseStep):
    ...
    """
    job_file = trun.Parameter()
    extra_files = trun.Parameter(default="")
    job_args = trun.Parameter(default="")
...
class CreateDataprocClusterStep(_DataprocBaseStep):
    ...
    gcloud_zone = trun.Parameter(default="europe-west1-c")
    gcloud_network = trun.Parameter(default="default")

    master_node_type = trun.Parameter(default="n1-standard-2")
    master_disk_size = trun.Parameter(default="100")
    worker_node_type = trun.Parameter(default="n1-standard-2")
    worker_disk_size = trun.Parameter(default="100")
    worker_normal_count = trun.Parameter(default="2")
    worker_preemptible_count = trun.Parameter(default="0")
    image_version = trun.Parameter(default="")
