"""
Contains some helper functions to run trund in daemon mode
"""
...
server_logger = logging.getLogger("trun.server")
...
def daemonize(cmd, pidfile=None, logdir=None, api_port=8082, address=None, unix_socket=None):
    ...
    logdir = logdir or "/var/log/trun"
    if not os.path.exists(logdir):
        os.makedirs(logdir)

    log_path = os.path.join(logdir, "trun-server.log")

    # redirect stdout/stderr
    today = datetime.date.today()
    stdout_path = os.path.join(
        logdir,
        "trun-server-{0:%Y-%m-%d}.out".format(today)
    )
    stderr_path = os.path.join(
        logdir,
        "trun-server-{0:%Y-%m-%d}.err".format(today)
    )
