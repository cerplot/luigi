function visualiserApp(trun) {
    ...
    function showStatusMessage(data) {
        ...
        var refreshInterval = setInterval(function() {
                if ($("#statusMessageModal").is(":hidden"))
                    ...
                    trun.getStepStatusMessage(data.stepId, function(data) {
                    ...
                    trun.getStepProgressPercentage(data.stepId, function(data) {
        ...
        $send.on("click", function($event) {
            ...
            if (content && data.worker) {
                if (awaitResponse) {
                    ...
                    trun.sendSchedulerMessage(data.worker, data.stepId, content, function(messageId) {
                        var interval = window.setInterval(function() {
                            trun.getSchedulerMessageResponse(data.stepId, messageId, function(response) {
                                if (response != null) {
                    ...
                    trun.sendSchedulerMessage(data.worker, data.stepId, content);
                }
            }
        });
        ...
        if (fragmentQuery.tab == "workers") {
            ...
            if (stepId) {
                ...
                if (fragmentQuery.invert) {
                    trun.getInverseDependencyGraph(stepId, depGraphCallback, !hideDone);
                } else {
                    trun.getDependencyGraph(stepId, depGraphCallback, !hideDone);
                }
            }
        ...
        if (visType === 'd3') {
            $('.node').click(function(event) {
                ...
                if ($(this).attr('class').match(/\bFAILED\b/)) {
                    trun.getErrorTrace(stepId, function (error) {
    ...
    function bindListEvents() {
        ...
        /*
          Note: The #filter-input element is used by TrunAPI to constrain requests to the server.
    ...
    function updateSteps() {
        ...
        var ajax1 = trun.getRunningStepList(function(runningSteps) {
            updateStepCategory(dt, 'RUNNING', runningSteps);
        });

        var ajax2 = trun.getBatchRunningStepList(function(batchRunningSteps) {
            updateStepCategory(dt, 'BATCH_RUNNING', batchRunningSteps);
        });

        var ajax3 = trun.getFailedStepList(function(failedSteps) {
            updateStepCategory(dt, 'FAILED', failedSteps);
        });

        var ajax4 = trun.getUpstreamFailedStepList(function(upstreamFailedSteps) {
            updateStepCategory(dt, 'UPSTREAM_FAILED', upstreamFailedSteps);
        });

        var ajax5 = trun.getDisabledStepList(function(disabledSteps) {
            updateStepCategory(dt, 'DISABLED', disabledSteps);
        });

        var ajax6 = trun.getUpstreamDisabledStepList(function(upstreamDisabledSteps) {
            updateStepCategory(dt, 'UPSTREAM_DISABLED', upstreamDisabledSteps);
        });

        var ajax7 = trun.getPendingStepList(function(pendingSteps) {
            updateStepCategory(dt, 'PENDING', pendingSteps);
        });

        var ajax8 = trun.getDoneStepList(function(doneSteps) {
    ...
    function updateWorkerProcesses(worker, n) {
        ...
        trun.setWorkerProcesses(worker, n, function() {
            $label.text(n);
        });
    }
    ...
    function updateResourceCount(resource, n) {
        ...
        trun.updateResource(resource, n, function() {
   ...
   function expandedResources() {
    ...
    $(document).ready(function() {
        ...
        trun.hasStepHistory(function(hasStepHistory) {
            if (hasStepHistory) {
        ...
        trun.isPauseEnabled(function(enabled) {
            if (enabled) {
                trun.isPaused(createPauseToggle);
            }
        });

        trun.getWorkerList(function(workers) {
        ...
        trun.getResourceList(function(resources) {
        ...
        $('#stepTable tbody').on('click', 'td.details-control .showError', function () {
            ...
            var row = dt.row( tr );
            var data = row.data();
            trun.getErrorTrace(data.stepId, function(error) {
                showErrorTrace(error);
            });
        } );
        ...
        $('#stepTable tbody').on('click', 'td.details-control .forgiveFailures', function (ev) {
            ...
            var row = dt.row( tr );
            var data = row.data();
            trun.forgiveFailures(data.stepId, function(data) {
                if (ev.altKey) {
        ...
        $('#stepTable tbody').on('click', 'td.details-control .markAsDone', function (ev) {
            ...
            var row = dt.row( tr );
            var data = row.data();
            trun.markAsDone(data.stepId, function(data) {
                if (ev.altKey) {
                    updateSteps(); // update may not be cheap
                } else {
                    that.tooltip('hide');
        ...
        $('#stepTable tbody').on('click', 'td.details-control .re-enable-button', function (ev) {
            var that = $(this);
            trun.reEnable(that.attr("data-step-id"), function(data) {
                if (ev.altKey) {
                    updateSteps(); // update may not be cheap
                } else {
                    that.tooltip('hide');
