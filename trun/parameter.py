''' Parameters are one of the core concepts of Trun.
All Parameters sit on :class:`~trun.step.Step` classes.
...
from trun import date_interval
from trun import step_register
...
class Parameter:
    ...
    def __init__(self, default=_no_value, is_global=False, significant=True, description=None,
        ...
        :param visibility: A Parameter whose value is a :py:class:`~trun.parameter.ParameterVisibility`.
    ...
    def next_in_enumeration(self, _value):
        ...
        :py:mod:`trun.execution_summary` module for pretty printing
...
class MonthParameter(DateParameter):
    ...
    :py:class:`~trun.date_interval.Month`.
    """
...
class YearParameter(DateParameter):
    ...
    :py:class:`~datetime.date` (ignoring the month and day values) or :py:class:`~trun.date_interval.Year`.
    """
...
class BoolParameter(Parameter):
    ...
    .. code-block:: python
        ...
        class MyStep(trun.Step):
            implicit_bool = trun.BoolParameter(parsing=trun.BoolParameter.IMPLICIT_PARSING)
            explicit_bool = trun.BoolParameter(parsing=trun.BoolParameter.EXPLICIT_PARSING)

    or globally by

    .. code-block:: python

        trun.BoolParameter.parsing = trun.BoolParameter.EXPLICIT_PARSING
...
class DateIntervalParameter(Parameter):
    """
    A Parameter whose value is a :py:class:`~trun.date_interval.DateInterval`.
    ...
    def parse(self, s):
        """
        Parses a :py:class:`~trun.date_interval.DateInterval` from the input.

        see :py:mod:`trun.date_interval`
          for details on the parsing of DateIntervals.
        """
        # TODO: can we use xml.utils.iso8601 or something similar?

        from trun import date_interval as d
...
class StepParameter(Parameter):
    """
    A parameter that takes another trun step class.

    When used programatically, the parameter should be specified
    directly with the :py:class:`trun.step.Step` (sub) class. Like
    ``MyMetaStep(my_step_param=my_steps.MyStep)``. On the command line,
    you specify the :py:meth:`trun.step.Step.get_step_family`. Like

    .. code-block:: console

            $ trun --module my_steps MyMetaStep --my_step_param my_namespace.MyStep

    Where ``my_namespace.MyStep`` is defined in the ``my_steps`` python module.

    When the :py:class:`trun.step.Step` class is instantiated to an object.
    The value will always be a step class (and not a string).
    """

    def parse(self, input):
        """
        Parse a step_famly using the :class:`~trun.step_register.Register`
        """
        return step_register.Register.get_step_cls(input)

    def serialize(self, cls):
        """
        Converts the :py:class:`trun.step.Step` (sub) class to its family name.
        """
...
class EnumParameter(Parameter):
    ...
    .. code-block:: python
        ...
        class MyStep(trun.Step):
          my_param = trun.EnumParameter(enum=Model)

    At the command line, use,

    .. code-block:: console

        $ trun --module my_steps MyStep --my-param Honda
...
class EnumListParameter(Parameter):
    ...
    .. code-block:: python
        ...
        class MyStep(trun.Step):
          my_param = trun.EnumListParameter(enum=Model)

    At the command line, use,

    .. code-block:: console

        $ trun --module my_steps MyStep --my-param Honda,Volvo
...
class DictParameter(Parameter):
    ...
    .. code-block:: console
        ...
        $ trun --module my_steps MyStep --tags <JSON string>

    Simple example with two tags:

    .. code-block:: console

        $ trun --module my_steps MyStep --tags '{"role": "web", "env": "staging"}'

    It can be used to define dynamic parameters, when you do not know the exact list of your parameters (e.g. list of
    tags, that are dynamically constructed outside Trun), or you have a complex parameter containing logically related
    ...
    .. code-block:: console
        ...
        $ trun --module my_steps MyStep --tags '{"role": "UNKNOWN_VALUE", "env": "staging"}'
    ...
    .. code-block:: python
        ...
        class MyStep(trun.Step):
          tags = trun.DictParameter(schema=custom_validator)
...
class ListParameter(Parameter):
    ...
    .. code-block:: console
        ...
        $ trun --module my_steps MyStep --grades <JSON string>

    Simple example with two grades:

    .. code-block:: console

        $ trun --module my_steps MyStep --grades '[100,70]'
    ...
    .. code-block:: console
        ...
        $ trun --module my_steps MyStep --numbers '[1, 8.7, 6]'

    while these commands will fail because the parameter is not valid:

    .. code-block:: console

        $ trun --module my_steps MyStep --numbers '[]'  # must have at least 1 element
        $ trun --module my_steps MyStep --numbers '[-999, 999]'  # elements must be in [0, 10]
    ...
    .. code-block:: python
        ...
        class MyStep(trun.Step):
          grades = trun.ListParameter(schema=custom_validator)
...
class TupleParameter(ListParameter):
    ...
    .. code-block:: python
        ...
        class MyStep(trun.Step):
          book_locations = trun.TupleParameter()
    ...
    .. code-block:: console
        ...
        $ trun --module my_steps MyStep --book_locations <JSON string>

    Simple example with two grades:

    .. code-block:: console

        $ trun --module my_steps MyStep --book_locations '((12,3),(4,15),(52,1))'
    """
...
class NumericalParameter(Parameter):
    ...
    .. code-block:: python
        ...
        class MyStep(trun.Step):
            my_param_1 = trun.NumericalParameter(
                var_type=int, min_value=-3, max_value=7) # -3 <= my_param_1 < 7
            my_param_2 = trun.NumericalParameter(
    ...
    .. code-block:: console
        ...
        $ trun --module my_steps MyStep --my-param-1 -3 --my-param-2 -2
    """
...
class ChoiceParameter(Parameter):
    ...
    .. code-block:: python
        ...
        class MyStep(trun.Step):
            my_param = trun.ChoiceParameter(choices=[0.1, 0.2, 0.3], var_type=float)

    At the command line, use

    .. code-block:: console

        $ trun --module my_steps MyStep --my-param 0.1

    Consider using :class:`~trun.EnumParameter` for a typed, structured
...
class PathParameter(Parameter):
    ...
    .. code-block:: python
        ...
        class MyStep(trun.Step):
            existing_file_path = trun.PathParameter(exists=True)
            new_file_path = trun.PathParameter()
    ...
    .. code-block:: console
        ...
        $ trun --module my_steps MyStep --existing-file-path <path> --new-file-path <path>
    """
