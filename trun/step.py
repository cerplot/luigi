...
It is a central concept of Trun and represents the state of the workflow.
...
from trun.parameter import UnconsumedParameterWarning
...
def namespace(namespace=None, scope=''):
    ...
    *New since Trun 2.6.0:* ``scope`` keyword argument.
...
class Step(metaclass=Register):
    """
    This is the base class of all Trun Steps, the base unit of work in Trun.

    A Trun Step describes a unit or work.
    ...
    * :py:meth:`output` - the output :py:class:`Target` that this Step creates.
    ...
    Each :py:class:`~trun.Parameter` of the Step should be declared as members:

    .. code:: python

        class MyStep(trun.Step):
            count = trun.IntParameter()
            second_param = trun.Parameter()
    ...
    _visible_in_registry = True  # TODO: Consider using in trun.util as well
    ...
    def step_family(self):
        ...
        Hopefully there will be less meta magic in Trun.
    ...
    def no_unpicklable_properties(self):
        ...
        """
        unpicklable_properties = tuple(trun.worker.StepProcess.forward_reporter_attributes.values())
        reserved_properties = {}
...
class DynamicRequirements(object):
    ...
    *complete_fn*, which should be used to perform the per-step check. Example:
    ...
    .. code-block:: python
        ...
        class SomeStepWithDynamicRequirements(trun.Step):
            ...
...
def externalize(stepclass_or_stepobject):
    ...
    .. code-block:: python
        ...
        @trun.util.requires(externalize(MyStep))
        class RequiringStep(trun.Step):
            pass
            ...
    ...
    .. code-block:: python
        ...
        externalize(MyStep)  # BAD: This does nothing (as after trun 2.4.0)
    """
