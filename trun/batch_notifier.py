"""
Library for sending batch notifications from the Trun scheduler. This module
is internal to Trun and not designed for use in other contexts.
"""
...
import trun
from trun.notifications import send_email, email
import trun.parameter


class batch_email(trun.Config):
    email_interval = trun.parameter.IntParameter(
        default=60,
    ...
    batch_mode = trun.parameter.ChoiceParameter(
...
class BatchNotifier:
    ...
    def _send_email(self, fail_counts, disable_counts, scheduling_counts, fail_expls, owner):
        ...
        if subject_base:
            ...
            subject = 'Trun: {}{} in the last {} minutes'.format(
