...
Most trun plumbers will find themselves in an awkward step parameter situation
at some point or another.  Consider the following "parameter explosion"
problem:

.. code-block:: python

    class StepA(trun.ExternalStep):
        param_a = trun.Parameter()

        def output(self):
            return trun.LocalTarget('/tmp/log-{t.param_a}'.format(t=self))

    class StepB(trun.Step):
        param_b = trun.Parameter()
        param_a = trun.Parameter()
...
**Approach 1**:  Parameters via command line or config instead of :func:`~trun.step.Step.requires`.
...
.. code-block:: bash
    ...
    trun --module my_steps StepC --param-c foo --StepB-param-b bar --StepA-param-a baz
...
.. code-block:: python
    ...
    class Params(trun.Config):
        param_c = trun.Parameter()
        param_b = trun.Parameter()
        param_a = trun.Parameter()

    class StepA(Params, trun.ExternalStep):
        def output(self):
            return trun.LocalTarget('/tmp/log-{t.param_a}'.format(t=self))
...
idempotent and atomic nature of trun steps.  Unnecessary significant step
...
derived from :class:`~trun.step.Step` should be undertaken with caution and avoided
where possible.


**Approach 3**: Use :class:`~trun.util.inherits` and :class:`~trun.util.requires`

The :class:`~trun.util.inherits` class decorator in this module copies parameters (and
...
The :class:`~trun.util.requires` helper function can reduce this pattern even further.   It
does everything :class:`~trun.util.inherits` does,
and also attaches a :class:`~trun.util.requires` method
to your step (still all without pythonic inheritance).

But how does it know how to invoke the upstream step?  It uses :func:`~trun.step.Step.clone`
behind the scenes!
...
class inherits:
    ...
    *New after Trun 2.7.6:* multiple arguments support.

    Usage:

    .. code-block:: python

        class AnotherStep(trun.Step):
            m = trun.IntParameter()

        class YetAnotherStep(trun.Step):
            n = trun.IntParameter()

        @inherits(AnotherStep)
        class MyFirstStep(trun.Step):
            def requires(self):
...
class requires:
    """
    Same as :class:`~trun.util.inherits`, but also auto-defines the requires method.
