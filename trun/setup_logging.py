...
#
"""
This module contains helper classes for configuring logging for trund and
...
from trun.configuration import get_config, TrunConfigParser
from trun.freezing import recursively_unfreeze
...
class BaseLogging:
    ...
    def _section(cls, opts):
        ...
        if isinstance(cls.config, TrunConfigParser):
            return False
        try:
...
class DaemonLogging(BaseLogging):
    ...
    def _cli(cls, opts):
        ...
        If `--logdir` -- set logging with next params:
            default Trun's formatter,
            INFO level,
            output in logdir in `trun-server.log` file
        """
        if opts.background:
        ...
        if opts.logdir:
            logging.basicConfig(
                ...
                filename=os.path.join(opts.logdir, "trun-server.log"))
            return True
