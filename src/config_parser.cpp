...
void processIncludedFiles(toml::table& merged_data, std::unordered_map<std::string, std::filesystem::path>& file_paths_dict, std::set<std::filesystem::path>& included_files, const std::string& include_chain) {
    for (const auto& [name, file_path] : file_paths_dict) {
        auto data = loadConfigFile(file_path, included_files, include_chain + " -> " + file_path.string());
        if (name == "_") {
...
std::shared_ptr<toml::table> loadConfigFile(
    ...
    if(node && node->is_table()) {
        ...
        processIncludedFiles(*config, file_paths_dict, included_files, include_chain);
...
std::shared_ptr<toml::table> validate_config_file(const std::filesystem::path& default_config_file_path, const std::filesystem::path& config_file_path) {
    ...
    auto default_config = loadConfigFile(default_config_file_path, included_files);
    auto config = loadConfigFile(config_file_path, included_files);
