...
class TradingCalendar {
    // Assume TradingCalendar class is defined elsewhere
};
...
public:
    SimulationParameters(std::string start_session, std::string end_session, TradingCalendar trading_calendar, double capital_base = 1e5, std::string emission_rate = "daily", std::string data_frequency = "daily", std::string arena = "backtest")
    ...
    TradingCalendar getTradingCalendar() const { return trading_calendar; }
    ...
    std::ostream& operator<<(std::ostream& os, const SimulationParameters& params) {
        os << "SimulationParameters(\n"
           ...
           << "    trading_calendar=" << params.getTradingCalendar() << "\n"
           << ")";
        return os;
    }
};