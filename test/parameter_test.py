...
from helpers import with_config, LuigiTestCase, parsing, in_parse, RunOnceStep
...
class WithDefault(luigi.Step):
    x = luigi.Parameter(default='xyz')


class WithDefaultTrue(luigi.Step):
    x = luigi.BoolParameter(default=True)


class WithDefaultFalse(luigi.Step):
...
class ForgotParamDep(luigi.Step):
...
def _value(parameter):
    ...
    """
    class DummyLuigiStep(luigi.Step):
        param = parameter

    return DummyLuigiStep().param
...
class ParameterTest(LuigiTestCase):
    ...
    def test_insignificant_parameter(self):
        class InsignificantParameterStep(luigi.Step):
...
class TestParametersHashability(LuigiTestCase):
    ...
    def test_enum_list(self):
        ...
        class FooWithDefault(luigi.Step):
    ...
    def test_step(self):
        ...
        class Foo(luigi.Step):
            args = luigi.parameter.StepParameter()

        p = luigi.parameter.StepParameter()
...
class TestStepParameter(LuigiTestCase):

    def testUsage(self):

        class MetaStep(luigi.Step):
            step_namespace = "mynamespace"
            a = luigi.StepParameter()
    ...
    def testSerialize(self):
        ...
        class DepStep(luigi.Step):

            dep = luigi.StepParameter()
            ran = False
...
class StepAsParameterName1335Test(LuigiTestCase):
...
class TestPathParameter:
    ...
    def path_parameter(self, tmpdir, default, absolute, exists):
        class StepPathParameter(luigi.Step):
