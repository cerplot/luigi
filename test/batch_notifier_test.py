...
class BatchNotifier(trun.batch_notifier.BatchNotifier):
...
class BatchNotifierTest(unittest.TestCase):
    def setUp(self):
        self.time_mock = mock.patch('trun.batch_notifier.time.time')
        self.time = self.time_mock.start()
        self.time.return_value = 0.0

        self.send_email_mock = mock.patch('trun.batch_notifier.send_email')
        self.send_email = self.send_email_mock.start()

        self.email_mock = mock.patch('trun.batch_notifier.email')
    ...
    def test_send_single_failure(self):
        ...
        bn.add_failure('Step(a=5)', 'Step', {'a': 5}, 'error', [])
        bn.send_email()
        self.check_email_send(
            'Trun: 1 failure in the last 60 minutes',
            '- Step(a=5) (1 failure)'
        )
    ...
    def test_send_single_failure_to_owner_only(self):
        self.email().receiver = None
        bn = BatchNotifier(batch_mode='all')
        bn.add_failure('Step(a=5)', 'Step', {'a': 5}, 'error', ['owner@test.com'])
        bn.send_email()
        self.check_email_send(
            'Trun: Your steps have 1 failure in the last 60 minutes',
            '- Step(a=5) (1 failure)',
    ...
    def test_send_single_disable(self):
        ...
        for _ in range(10):
            bn.add_failure('Step(a=5)', 'Step', {'a': 5}, 'error', [])
        bn.add_disable('Step(a=5)', 'Step', {'a': 5}, [])
        bn.send_email()
        self.check_email_send(
            'Trun: 10 failures, 1 disable in the last 60 minutes',
            '- Step(a=5) (10 failures, 1 disable)'
        )
    ...
    def test_send_multiple_disables(self):
        ...
        for _ in range(10):
            bn.add_failure('Step(a=5)', 'Step', {'a': 5}, 'error', [])
            bn.add_failure('Step(a=6)', 'Step', {'a': 6}, 'error', [])
        bn.add_disable('Step(a=5)', 'Step', {'a': 5}, [])
        bn.add_disable('Step(a=6)', 'Step', {'a': 6}, [])
        bn.send_email()
        self.check_email_send(
            'Trun: 20 failures, 2 disables in the last 60 minutes',
            '- Step (20 failures, 2 disables)'
        )

    def test_send_single_scheduling_fail(self):
        bn = BatchNotifier(batch_mode='family')
        bn.add_scheduling_fail('Step()', 'Step', {}, 'error', [])
        bn.send_email()
        self.check_email_send(
            'Trun: 1 scheduling failure in the last 60 minutes',
            '- Step (1 scheduling failure)',
        )
    ...
    def test_multiple_failures_of_same_job(self):
        ...
        bn.send_email()
        self.check_email_send(
            'Trun: 3 failures in the last 60 minutes',
            '- Step(a=5) (3 failures)'
        )
    ...
    def test_group_on_family(self):
        ...
        bn.add_failure('Step(a=6)', 'Step', {'a': 6}, 'error', [])
        bn.add_failure('Step(a=6)', 'Step', {'a': 6}, 'error', [])
        bn.add_failure('OtherStep(a=6)', 'OtherStep', {'a': 6}, 'error', [])
        bn.send_email()
        self.check_email_send(
            'Trun: 4 failures in the last 60 minutes',
            '- Step (3 failures)\n'
            '- OtherStep (1 failure)'
        )
    ...
    def test_group_on_unbatched_params(self):
        ...
        bn.add_failure('Step(a=5, b=1)', 'Step', {'a': 5}, 'error', [])
        bn.add_failure('Step(a=5, b=2)', 'Step', {'a': 5}, 'error', [])
        bn.add_failure('Step(a=6, b=1)', 'Step', {'a': 6}, 'error', [])
        bn.add_failure('Step(a=6, b=2)', 'Step', {'a': 6}, 'error', [])
        bn.add_failure('Step(a=6, b=3)', 'Step', {'a': 6}, 'error', [])
        bn.add_failure('Step(a=6, b=4)', 'Step', {'a': 6}, 'error', [])
        bn.add_failure('OtherStep(a=5, b=1)', 'OtherStep', {'a': 5}, 'error', [])
        bn.add_failure('OtherStep(a=6, b=1)', 'OtherStep', {'a': 6}, 'error', [])
        bn.add_failure('OtherStep(a=6, b=2)', 'OtherStep', {'a': 6}, 'error', [])
        bn.add_failure('OtherStep(a=6, b=3)', 'OtherStep', {'a': 6}, 'error', [])
        bn.send_email()
        self.check_email_send(
            'Trun: 10 failures in the last 60 minutes',
            '- Step(a=6) (4 failures)\n'
            '- OtherStep(a=6) (3 failures)\n'
            '- Step(a=5) (2 failures)\n'
            '- OtherStep(a=5) (1 failure)'
        )

    def test_include_one_expl_includes_latest(self):
        bn = BatchNotifier(batch_mode='family', error_messages=1)
        bn.add_failure('Step(a=1)', 'Step', {'a': 1}, 'error 1', [])
        bn.add_failure('Step(a=2)', 'Step', {'a': 2}, 'error 2', [])
        bn.add_failure('StepB(a=1)', 'StepB', {'a': 1}, 'error', [])
    ...
    def test_limit_expl_length(self):
        ...
        bn.add_failure('Step(a=1)', 'Step', {'a': '1'}, 'line 1\nline 2\nline 3\nline 4\n', [])
        bn.send_email()
        self.check_email_send(
            'Trun: 1 failure in the last 60 minutes',
            '- Step (1 failure)\n'
    ...
    def test_expl_varies_by_owner(self):
        bn = BatchNotifier(batch_mode='family', error_messages=1)
        bn.add_failure('Step(a=1)', 'Step', {'a': '1'}, 'msg1', owners=['a@test.com'])
        bn.add_failure('Step(a=2)', 'Step', {'a': '2'}, 'msg2', owners=['b@test.com'])
        bn.send_email()
        send_calls = [
            mock.call(
                'Trun: Your steps have 1 failure in the last 60 minutes',
            ...
            mock.call(
                'Trun: 2 failures in the last 60 minutes',
                '- Step (2 failures)\n'
                '\n'
                '      msg2',
                'sender@test.com',
    ...
    def test_send_clears_backlog(self):
        ...
        bn.add_failure('Step(a=5)', 'Step', {'a': 5}, 'error', [])
        bn.add_disable('Step(a=5)', 'Step', {'a': 5}, [])
        bn.add_scheduling_fail('Step(a=6)', 'Step', {'a': 6}, 'scheduling error', [])
        bn.send_email()
    ...
    def test_email_gets_cleared_on_failure(self):
        ...
        bn.add_failure('Step(a=5)', 'Step', {'a': 1}, '', [])
    ...
    def test_send_clears_all_old_data(self):
        ...
        for i in range(100):
            bn.add_failure('Step(a=5)', 'Step', {'a': 5}, 'error {}'.format(i), [])
            bn.add_disable('Step(a=5)', 'Step', {'a': 5}, [])
            bn.add_scheduling_fail('Step(a=6)', 'Step', {'a': 6}, 'scheduling error {}'.format(i), [])
            bn.send_email()
            self.check_email_send(
                'Trun: 1 failure, 1 disable, 1 scheduling failure in the last 60 minutes',
                '- Step(a=5) (1 failure, 1 disable)\n'
                '\n'
                '      error {}\n'
                '\n'
                '- Step(a=6) (1 scheduling failure)\n'
                '\n'
    ...
    def test_auto_send_on_update_after_time_period_with_disable_only(self):
        ...
        bn.update()
        self.check_email_send(
            'Trun: 1 disable in the last 60 minutes',
            '- Step(a=5) (1 disable)'
        )
    ...
    def test_send_batch_failure_emails_to_owners(self):
        bn = BatchNotifier(batch_mode='all')
        bn.add_failure('Step(a=1)', 'Step', {'a': '1'}, 'error', ['a@test.com', 'b@test.com'])
        bn.add_failure('Step(a=1)', 'Step', {'a': '1'}, 'error', ['b@test.com'])
        bn.add_failure('Step(a=2)', 'Step', {'a': '2'}, 'error', ['a@test.com'])
        bn.send_email()
        ...
        send_calls = [
            ...
            mock.call(
                'Trun: Your steps have 2 failures in the last 60 minutes',
                '- Step(a=1) (1 failure)\n'
                '- Step(a=2) (1 failure)',
                'sender@test.com',
                ('a@test.com',),
    ...
    def test_send_batch_disable_email_to_owners(self):
        bn = BatchNotifier(batch_mode='all')
        bn.add_disable('Step(a=1)', 'Step', {'a': '1'}, ['a@test.com'])
        bn.send_email()
        ...
        send_calls = [
            ...
            mock.call(
                'Trun: Your steps have 1 disable in the last 60 minutes',
                '- Step(a=1) (1 disable)',
                'sender@test.com',
                ('a@test.com',),
            ),
    ...
    def test_batch_identical_expls(self):
        ...
        bn.add_failure('Step(a=1)', 'Step', {'a': '1'}, 'msg1', [])
        bn.add_failure('Step(a=2)', 'Step', {'a': '2'}, 'msg1', [])
        bn.add_failure('Step(a=3)', 'Step', {'a': '3'}, 'msg1', [])
        bn.add_failure('Step(a=4)', 'Step', {'a': '4'}, 'msg2', [])
        bn.add_failure('Step(a=4)', 'Step', {'a': '4'}, 'msg2', [])
        bn.send_email()
        self.check_email_send(
            'Trun: 5 failures in the last 60 minutes',
            '- Step(a=1) (1 failure)\n'
            '  Step(a=2) (1 failure)\n'
            '  Step(a=3) (1 failure)\n'
            '\n'
    ...
    def test_batch_identical_expls_html(self):
        ...
        self.check_email_send(
            'Trun: 5 failures in the last 60 minutes',
            '<ul>\n'
            '<li>Step(a=1) (1 failure)\n'
            '<br>Step(a=2) (1 failure)\n'
            '<br>Step(a=3) (1 failure)\n'
            '<pre>msg1</pre>\n'
            '<li>Step(a=4) (2 failures)\n'
            '<pre>msg2</pre>\n'
            '</ul>'
        )
    ...
    def test_unicode_error_message(self):
        ...
        bn.add_failure('Step()', 'Step', {}, 'Érror', [])
        bn.send_email()
        self.check_email_send(
            'Trun: 1 failure in the last 60 minutes',
            '- Step() (1 failure)\n'
    ...
    def test_unicode_param_value(self):
        for batch_mode in ('all', 'unbatched_params'):
            ...
            bn.add_failure('Step(a=á)', 'Step', {'a': 'á'}, 'error', [])
            bn.send_email()
            self.check_email_send(
                'Trun: 1 failure in the last 60 minutes',
                '- Step(a=á) (1 failure)'
    ...
    def test_unicode_param_name(self):
        for batch_mode in ('all', 'unbatched_params'):
            self.send_email.reset_mock()
            bn = BatchNotifier(batch_mode=batch_mode)
            bn.add_failure('Step(á=a)', 'Step', {'á': 'a'}, 'error', [])
            bn.send_email()
            self.check_email_send(
                'Trun: 1 failure in the last 60 minutes',
                '- Step(á=a) (1 failure)'
