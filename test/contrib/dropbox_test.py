...
import trun
from trun.format import NopFormat

try:
    import dropbox
    import dropbox.exceptions
    from trun.contrib.dropbox import DropboxClient
except ImportError:
...
DROPBOX_TEST_PATH = "/trun-tests/trun-test-" + datetime.now().strftime("%Y.%m.%d-%H.%M.%S") + str(uuid.uuid4())
...
class TestClientDropbox(unittest.TestCase):
    def setUp(self):
        self.trunconn = DropboxClient(DROPBOX_APP_TOKEN)
    ...
    def test_exists(self):
        self.assertTrue(self.trunconn.exists('/'))
        self.assertTrue(self.trunconn.exists(DROPBOX_TEST_PATH))
        self.assertTrue(self.trunconn.exists(DROPBOX_TEST_SIMPLE_DIR))
        self.assertTrue(self.trunconn.exists(DROPBOX_TEST_SIMPLE_DIR + '/'))
        self.assertTrue(self.trunconn.exists(DROPBOX_TEST_SIMPLE_FILE))

        self.assertFalse(self.trunconn.exists(DROPBOX_TEST_SIMPLE_FILE + '/'))
        self.assertFalse(self.trunconn.exists(DROPBOX_TEST_NON_EXISTING_FILE))

    def test_listdir_simple(self):
        list_of_dirs = self.trunconn.listdir(DROPBOX_TEST_PATH)
    ...
    def test_listdir_multiple(self):
        list_of_dirs = self.trunconn.listdir(DROPBOX_TEST_PATH, limit=2)
        self.assertTrue('/' not in list_of_dirs)
        self.assertTrue(DROPBOX_TEST_PATH in list_of_dirs)
        self.assertTrue(DROPBOX_TEST_SIMPLE_FILE in list_of_dirs)  # we verify recursivity
    ...
    def test_listdir_nonexisting(self):
        with self.assertRaises(dropbox.exceptions.ApiError):
            self.trunconn.listdir(DROPBOX_TEST_NON_EXISTING_FILE)

    def test_remove(self):
        # We remove File_to_delete_1. We make sure it is the only file that gets deleted
        self.assertTrue(self.trunconn.exists(DROPBOX_TEST_FILE_TO_DELETE_1))
        self.assertTrue(self.trunconn.exists(DROPBOX_TEST_FILE_TO_DELETE_2))
        self.assertTrue(self.trunconn.remove(DROPBOX_TEST_FILE_TO_DELETE_1))
        self.assertFalse(self.trunconn.exists(DROPBOX_TEST_FILE_TO_DELETE_1))
        self.assertTrue(self.trunconn.exists(DROPBOX_TEST_FILE_TO_DELETE_2))

        # We remove a directory, we make sure that the files that were in the directory are also deleted
        self.trunconn.remove(DROPBOX_TEST_DIR_TO_DELETE)
        self.assertFalse(self.trunconn.exists(DROPBOX_TEST_FILE_TO_DELETE_2))

        # We make sure that we return False when we fail to remove a non-existing path
        self.assertFalse(self.trunconn.remove(DROPBOX_TEST_NON_EXISTING_FILE))
        self.assertFalse(self.trunconn.remove(DROPBOX_TEST_NON_EXISTING_FILE + '/'))

    def test_mkdir_new_dir(self):
        self.assertFalse(self.trunconn.exists(DROPBOX_TEST_DIR_TO_CREATE))
        self.assertFalse(self.trunconn.exists(DROPBOX_TEST_OUTER_DIR_TO_CREATE))
        self.trunconn.mkdir(DROPBOX_TEST_DIR_TO_CREATE)
        self.assertTrue(self.trunconn.isdir(DROPBOX_TEST_OUTER_DIR_TO_CREATE))
        self.assertTrue(self.trunconn.isdir(DROPBOX_TEST_DIR_TO_CREATE))
        self.assertTrue(self.trunconn.isdir(DROPBOX_TEST_DIR_TO_CREATE))

    def aux_lifecycle_of_directory(self, path):
        # Initially, the directory does not exists
        self.assertFalse(self.trunconn.exists(path))
        self.assertFalse(self.trunconn.isdir(path))

        # Now we create the directory and verify that it exists
        self.trunconn.mkdir(path)
        self.assertTrue(self.trunconn.exists(path))
        self.assertTrue(self.trunconn.isdir(path))

        # Now we remote the directory and verify that it no longer exists
        self.trunconn.remove(path)
        self.assertFalse(self.trunconn.exists(path))
        self.assertFalse(self.trunconn.isdir(path))
    ...
    def test_lifecycle_of_dirpath_with_several_trailing_mixed(self):
        self.trunconn.mkdir(DROPBOX_TEST_SIMPLE_DIR_TO_CREATE_AND_DELETE + '/')
        self.assertTrue(self.trunconn.exists(DROPBOX_TEST_SIMPLE_DIR_TO_CREATE_AND_DELETE))
        self.trunconn.remove(DROPBOX_TEST_SIMPLE_DIR_TO_CREATE_AND_DELETE)
        self.assertFalse(self.trunconn.exists(DROPBOX_TEST_SIMPLE_DIR_TO_CREATE_AND_DELETE + '/'))
    ...
    def test_mkdir_recreate_dir(self):
        try:
            self.trunconn.mkdir(DROPBOX_TEST_SIMPLE_DIR)
        except Exception as ex:
            self.fail("mkdir with default options raises Exception:" + str(ex))

        try:
            self.trunconn.mkdir(DROPBOX_TEST_SIMPLE_DIR, raise_if_exists=False)
        except Exception as ex:
            self.fail("mkdir with 'raise_if_exists=False' raises Exception:" + str(ex))

        with self.assertRaises(trun.target.FileAlreadyExists):
            self.trunconn.mkdir(DROPBOX_TEST_SIMPLE_DIR, raise_if_exists=True)
    ...
    def test_mkdir_recreate_file(self):
        with self.assertRaises(trun.target.NotADirectory):
            self.trunconn.mkdir(DROPBOX_TEST_SIMPLE_FILE)

        with self.assertRaises(trun.target.NotADirectory):
            self.trunconn.mkdir(DROPBOX_TEST_SIMPLE_FILE, raise_if_exists=True)

        with self.assertRaises(trun.target.NotADirectory):
            self.trunconn.mkdir(DROPBOX_TEST_SIMPLE_FILE, raise_if_exists=False)

    def test_isdir(self):
        self.assertTrue(self.trunconn.isdir('/'))
        self.assertTrue(self.trunconn.isdir(DROPBOX_TEST_PATH))
        self.assertTrue(self.trunconn.isdir(DROPBOX_TEST_SIMPLE_DIR))
        self.assertTrue(self.trunconn.isdir(DROPBOX_TEST_SIMPLE_DIR + '/'))

        self.assertFalse(self.trunconn.isdir(DROPBOX_TEST_SIMPLE_FILE))
        self.assertFalse(self.trunconn.isdir(DROPBOX_TEST_NON_EXISTING_FILE))
        self.assertFalse(self.trunconn.isdir(DROPBOX_TEST_NON_EXISTING_FILE + '/'))
    ...
    def test_move(self):
        ...
        self.trunconn.move(DROPBOX_TEST_FILE_TO_MOVE_ORIG, DROPBOX_TEST_FILE_TO_MOVE_DEST)
        ...
        self.assertFalse(self.trunconn.exists(DROPBOX_TEST_FILE_TO_MOVE_ORIG))
        self.assertTrue(self.trunconn.exists(DROPBOX_TEST_FILE_TO_MOVE_DEST))

    def test_copy(self):
        md, res = self.dropbox_api.files_download(DROPBOX_TEST_FILE_TO_COPY_ORIG)
        initial_contents = res.content

        self.trunconn.copy(DROPBOX_TEST_FILE_TO_COPY_ORIG, DROPBOX_TEST_FILE_TO_COPY_DEST)
        ...
        self.assertTrue(self.trunconn.exists(DROPBOX_TEST_FILE_TO_COPY_ORIG))
        self.assertTrue(self.trunconn.exists(DROPBOX_TEST_FILE_TO_COPY_DEST))
...
class TestDropboxTarget(unittest.TestCase):
    ...
    def test_download_from_dropboxtarget_to_local(self):
        class Download(trun.ExternalStep):
            dbx_path = trun.Parameter()

            def output(self):
                return trun.contrib.dropbox.DropboxTarget(self.dbx_path, DROPBOX_APP_TOKEN, format=NopFormat())

        class DbxToLocalStep(trun.Step):
            local_path = trun.Parameter()
            dbx_path = trun.Parameter()
            ...
            def output(self):
                return trun.LocalTarget(path=self.local_path, format=NopFormat())
        ...
        trun.build([DbxToLocalStep(dbx_path=DROPBOX_TEST_SIMPLE_FILE, local_path=tmp_file)],
    ...
    def test_write_small_text_file_to_dropbox(self):
        ...
        class WriteToDrobopxTest(trun.Step):
            def output(self):
                return trun.contrib.dropbox.DropboxTarget(DROPBOX_TEST_FILE_TO_UPLOAD_TEXT, DROPBOX_APP_TOKEN)

            def run(self):
                with self.output().open('w') as dbxfile:
                    dbxfile.write(small_input_text)

        trun.build([WriteToDrobopxTest()], local_scheduler=True)
    ...
    def aux_write_binary_file_to_dropbox(self, multiplier):
        ...
        class WriteToDrobopxTest(trun.Step):
            def output(self):
                return trun.contrib.dropbox.DropboxTarget(output_file, DROPBOX_APP_TOKEN,
                                                           format=trun.format.Nop)
    ...
    def test_write_using_nondefault_format(self):
        ...
        class WriteToDrobopxTest(trun.Step):
            def output(self):
                return trun.contrib.dropbox.DropboxTarget(DROPBOX_TEST_FILE_TO_UPLOAD_BZIP2, DROPBOX_APP_TOKEN,
                                                           format=trun.format.Bzip2)
    ...
    def test_write_using_a_temporary_path(self):
        ...
        class WriteToDrobopxTest(trun.Step):
            def output(self):
                return trun.contrib.dropbox.DropboxTarget(DROPBOX_TEST_FILE_TO_UPLOAD_BIN, DROPBOX_APP_TOKEN)
